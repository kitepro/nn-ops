...
DLLEXPORT void avx2_gemm(float* A, float* B, float* C, int M, int N, int K, int trans_a, int trans_b, float* tempmem) {
	...
	if (trans_a <= 0 && trans_b <= 0) {
		...
		float* Apool = (float*)_aligned_malloc(sizeof(float) * Mb * Mr * KB * MR, 32);
		float* Bp = (float*)_aligned_malloc(sizeof(float) * Nr * KB * NR, 32);
		for (int kb = 0; kb < Kb; kb++) {
			...
			#pragma omp parallel for
			for (int mb = 0; mb < Mb; mb++) {
				float* Ap = Apool + mb * Mr * KB * MR;//(float*)_aligned_malloc(sizeof(float) * Mr * KB * MR, 32);
		...
		if (Kh != 0) {
			...
			#pragma omp parallel for
			for (int mb = 0; mb < Mb; mb++) {
				float* Ap = Apool + mb * Mr * KB * MR;// (float*)_aligned_malloc(sizeof(float)* Mr* Kh* MR, 32);

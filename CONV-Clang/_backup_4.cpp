				...
				for (int nw = 0; nw < NW; nw++) {
					for (int y = 0; y < Ky; y++) {
						...
						for (int n = 0; n < N; n++) {
							float s = 0;
							for (int c = 0; c < C; c++) {
								s += img[b * HWC + h * WC + w * C + c] * kern[x * KyNC + y * NC + n * C + c];
							}
							out[b * NHNWN + nh * NWN + nw * N + n] += s;
						}
					}
				}
			}
				...
				for (int nw = 0; nw < NW; nw++) {
					for (int y = 0; y < Ky; y++) {
						...
						for (int c = 0; c < C; c++) {
							float s = 0;
							for (int n = 0; n < N; n++) {
								s += out[b * NHNWN + nh * NWN + nw * N + n] * kern[x * KyNC + y * NC + n * C + c];
							}
							img[b * HWC + h * WC + w * C + c] += s;
						}
					}
				}
			}
				...
				for (int c = 0; c < C; c++) {
					...
					for (int b = 0; b < batchsize; b++) {
						for (int nh = 0; nh < NH; nh++) {
							...
							for (int nw = 0; nw < NW; nw++) {
								int w = nw * stride + y;
								s += img[b * HWC + h * WC + w * C + c] * out[b * NHNWN + nh * NWN + nw * N + n];
							}
						}
					}
					kern[x * KyNC + y * NC + n * C + c] += s;
				}
			}
		}
	}
